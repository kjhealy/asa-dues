...
### --------------------------------------###
### Clean up any objects from previous session
### --------------------------------------###
rm(list = ls(all = TRUE))
...
### Get sliding-scale dues data in long-form and clean up the names
...
### Data on fixed-fee rates for students and the unemployed
dues.specials <- read.csv("data/fixed-fee-table.csv", header=TRUE)

## Break out unemployed, order from highest to lowest cost
ind.u <- order(dues.specials$Unemployed)
dues.unemp <- dues.specials[ind.u,c("Association","Unemployed")]
...
dues.unemp[,"Association"] <- reorder.factor(dues.unemp[,"Association"],
                                             new.order=dues.specials$Association[ind.u])
